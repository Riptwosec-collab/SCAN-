(function(){
  ...
  function detectDocumentType(text,hint='general'){
    ...
    if(/บันทึกข้อความ|ส่วนราชการ|ผู้อำนวยการ|ผู้อํานวยการ|ราชการ|สรรพากร/.test(source))return 'government';
  ...
  function extractMoney(text){
    return unique((String(text||'').match(/(?:฿|THB)?\s*\d{1,3}(?:[, ]\d{3})*(?:\.\d{2})?|\d+\.\d{2}\s*(?:บาท|THB)?/g)||[]).map(normalize));
  }

  function extractDates(text){
    return unique((String(text||'').match(/\b\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4}\b|\b\d{1,2}\s+[ก-ฮ]+\s+\d{4}\b|\b[๑-๙0-9]{1,2}\s+[ก-ฮ]+\s+[๑-๙0-9]{4}\b/g)||[]).map(normalize));
  }

  function extractReceiptFields(text){
    const rows=lines(text);
    const money=extractMoney(text);
    const date=extractDates(text)[0]||'';
    ...
    const items=rows.filter(row=>/\d/.test(row)&&!/(total|ยอดรวม|vat|tax|change|cash)/i.test(row)).slice(0,30).map(row=>{
      const price=(row.match(/\d[\d,]*(?:\.\d{2})?\s*$/)||[''])[0];
      const qty=(row.match(/\b\d+(?:\.\d+)?\s*(?:x|ชิ้น|ea)?\b/i)||[''])[0];
      const name=normalize(row.replace(price,'').replace(qty,'')).slice(0,120);
      return {name,qty:normalize(qty),price:normalize(price)};
    }).filter(item=>item.name||item.price);
    return {storeName:rows[0]||'',receiptNo,date,items,subtotal:'',discount:'',total};
  }

  function extractInvoiceFields(text){
    const base=extractReceiptFields(text);
    return {...base,invoiceNo:firstMatch(text,[/(?:invoice|เลขที่ใบกำกับ|tax invoice no)\s*[:#：]?\s*([A-Z0-9\-\/]+)/i]),taxId:firstMatch(text,[/(?:tax id|เลขประจำตัวผู้เสียภาษี|เลขผู้เสียภาษี)\s*[:：]?\s*([0-9\-]{10,20})/i])};
  }

  function extractTicketFields(text){
    return {
      ticketNo:firstMatch(text,[/(?:ticket|incident|case)\s*(?:no|id|#)?\s*[:#：]?\s*([A-Z0-9\-]+)/i]),
      system:firstMatch(text,[/(?:system|ระบบ)\s*[:：]\s*([^\n]+)/i]),
      issue:firstMatch(text,[/(?:issue|problem|อาการ|ปัญหา)\s*[:：]\s*([^\n]+)/i]),
      ipAddress:unique(String(text||'').match(/\b(?:\d{1,3}\.){3}\d{1,3}\b/g)||[]),
      vlan:unique(String(text||'').match(/\bVlan\s*\d+\b|\bVLAN\s*\d+\b/gi)||[]),
      device:unique(String(text||'').match(/\b(?:router|switch|firewall|server|ap|controller)\b/gi)||[]),
      interface:unique(String(text||'').match(/\b(?:Gi|Fa|Te|Eth|Po)\s*\d+(?:\/\d+){0,3}\b/gi)||[]),
      severity:firstMatch(text,[/(?:severity|priority|ความรุนแรง)\s*[:：]\s*([^\n]+)/i]),
      requester:firstMatch(text,[/(?:requester|ผู้แจ้ง|user)\s*[:：]\s*([^\n]+)/i]),
      time:extractDates(text)[0]||''
    };
  }

  function extractEmailFields(text){
    const urls=unique(String(text||'').match(/https?:\/\/[^\s)]+/g)||[]);
    const emails=unique(String(text||'').match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi)||[]);
    return {from:firstMatch(text,[/^from\s*[:：]\s*(.+)$/im]),to:firstMatch(text,[/^to\s*[:：]\s*(.+)$/im]),subject:firstMatch(text,[/^subject\s*[:：]\s*(.+)$/im]),date:extractDates(text)[0]||'',urls,actionItems:lines(text).filter(line=>/please|action|required|ดำเนินการ|กรุณา|โปรด/i.test(line)).slice(0,20),emails};
  }

  function extractTableFields(text){
    const rows=lines(text);
    const tableRows=rows.map(row=>row.includes('|')?row.split('|').map(normalize).filter(Boolean):row.split(/\t| {2,}/).map(normalize).filter(Boolean)).filter(cols=>cols.length>1);
    return {headers:tableRows[0]||[],rows:tableRows.slice(1),rowCount:Math.max(0,tableRows.length-1)};
  }

  function extractGovernmentFields(text){
    return {
      department:firstMatch(text,[/ส่วนราชการ\s*[:：]?\s*(.+?)(?:\s+ที่\s|\s+วันที่\s|$)/i]),
      documentNo:firstMatch(text,[/(?:ที่|เลขที่)\s*[:：]?\s*([ก-ฮA-Z0-9\/\-.]+)/i]),
      date:extractDates(text)[0]||'',
  ...
  function extractFields(text,typeHint){
    ...
    if(documentType==='government')return {documentType,fields:extractGovernmentFields(text)};
