(function(){
  ...
  async function runThaiRescueRetry(file,passConfig,before){
    ...
    const attempts=[
      {...passConfig,lang:'tha',psm:'SINGLE_BLOCK',engineName:passConfig.engineName+' Thai-only retry'},
      {...passConfig,lang:'tha+eng',psm:'SPARSE_TEXT',engineName:passConfig.engineName+' sparse retry'}
    ];
    const results=[];
    for(const cfg of attempts){
      try{results.push({...await runTesseract(prep.blob,cfg),preprocessingUsed:prep.preprocessingUsed,retry:{retryUsed:true,retryReason:'Thai mojibake/low confidence',beforeConfidence:before.confidence||0}});}catch(error){results.push({confidence:0,text:'',error:error.message||String(error)});}
    }
  ...
  async function runOCRPass(file,passConfig,options={}){
    ...
    try{
      ...
      let retryMeta=null;
      if((data.confidence<60||data.mojibakeDetected)&&options.autoRetry!==false){
        const retry=data.mojibakeDetected?await runThaiRescueRetry(file,passConfig,data):await retryLowConfidenceOCR({...base,...data,preprocessingUsed:prep.preprocessingUsed},file,passConfig);
        if(retry&&retry.confidence>data.confidence&&!retry.mojibakeDetected){retryMeta=retry.retry;data=retry;}
      }
      const status=data.mojibakeDetected?'failed':'success';
      const error=data.mojibakeDetected?'Thai OCR mojibake detected: rejected this OCR pass':'listo';
      return {...base,...data,status,durationMs:Math.round(now()-started),preprocessingUsed:data.preprocessingUsed||prep.preprocessingUsed,retryUsed:!!retryMeta,retry:retryMeta||null,error:status==='failed'?error:''};
    }catch(error){
  ...
  async function retryLowConfidenceOCR(result,file,passConfig){
    ...
    const data=await runTesseract(prep.blob,{...passConfig,lang:passConfig.engineId==='ocr-2'?'tha':'tha+eng',psm:passConfig.engineId==='ocr-4'?'SPARSE_TEXT':'SINGLE_BLOCK'});
