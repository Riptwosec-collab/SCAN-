// RIPTWOSEC.SCAN Phase 9 Thai OCR Accuracy Upgrade
// Features: Vision recheck, OCR voting, Thai correction, layout parser, user learning.
...
function injectAccuracyPanel(){
  ...
  panel.open=!!s.open;
  panel.innerHTML=`
    <summary>Thai Accuracy Upgrade <span style="color:#93c5fd;font-size:12px">Vision + Voting + Learn</span></summary>
    <div class="ai-review-inner">
      ...
      <div class="hint">Vision Recheck ต้องมี Donate Access Code และตั้งค่า Backend /api/vision-ocr แล้ว</div>
    </div>`;
...
function parseDocumentLayout(text){
  const type=detectLayoutType(text);let out=String(text||'').replace(/[ \t]+\n/g,'\n').replace(/\n{3,}/g,'\n\n').trim();
...
function ocrVotingMerge(candidates){
  ...
  for(const cand of list){
    for(const raw of String(cand.text).split('\n')){
      const fixed=applyAdvancedThaiSpell(raw).trim();
      const key=fixed.replace(/\s+/g,'').toLowerCase();
      if(!fixed||seen.has(key)||lineQualityScore(fixed)<2)continue;
      lines.push(fixed);seen.add(key);
    }
  }
  return parseDocumentLayout(applyAdvancedThaiSpell(lines.join('\n')));
}
function getCurrentSourceCanvas(){return AppState.preparedCanvas||AppState.processedCanvas||AppState.imageCanvas||null}
...
async function visionRecheckIfEnabled(rawText,cleanedText,baseText){
  const settings=getAccuracySettings();if(!settings.vision)return baseText;
  const aiSettings=typeof getAiReviewSettings==='function'?getAiReviewSettings():{};const accessCode=(aiSettings.accessCode||'').trim();
  ...
  const res=await fetch('/api/vision-ocr',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({accessCode,imageDataUrl:canvasToJpegDataUrl(canvas),rawText:rawText||'',cleanedText:cleanedText||'',model:(aiSettings.model||'gpt-4o-mini').trim()})});
  const data=await res.json();if(!res.ok)throw new Error(data.error||'Vision OCR error');
  const vision=parseDocumentLayout(applyAdvancedThaiSpell(data.text||''));
  return textScoreForAccuracy(vision)>textScoreForAccuracy(baseText)+8?vision:baseText;
}
async function finalAccuracyUpgrade(rawText,cleanedText){
  const settings=getAccuracySettings();let out=cleanedText||rawText||'';
  if(settings.voting!==false&&AppState.ocrCandidates&&AppState.ocrCandidates.length){const voted=ocrVotingMerge(AppState.ocrCandidates);if(voted&&textScoreForAccuracy(voted)>textScoreForAccuracy(out)-4)out=voted}
  if(settings.thaiSpell!==false)out=applyAdvancedThaiSpell(out);
  if(settings.layout!==false)out=parseDocumentLayout(out);
...
function learnFromEditedOutput(){
  ...
  for(const bad of rawWords){if(editWords.includes(bad))continue;const good=editWords.find(w=>w[0]===bad[0]&&Math.abs(w.length-bad.length)<=3&&lineQualityScore(w)>lineQualityScore(bad)+2);if(good&&!rules.some(r=>r.from===bad))rules.push({from:bad,to:good,createdAt:Date.now()})}
...
(function patchAccuracyPipeline(){
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',patch);else patch();setTimeout(patch,500);
})();
