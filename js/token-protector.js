...
function protectImportantTokens(text){
  ...
  const patterns=[
    ...
    {type:'phone',regex:/\b0\d{1,2}[-\s]?\d{3}[-\s]?\d{4}\b/g},
    {type:'vlan',regex:/\bVLAN\s*\d{1,4}\b/gi},
    {type:'interface',regex:/\b(?:Gi|Te|Fa|Eth|Ethernet|GigabitEthernet|FastEthernet|Port-channel|Po)\s*\d+(?:\/\d+){0,3}\b/gi},
