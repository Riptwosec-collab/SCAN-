...
function fixThaiWords(text){
  let out=text;
  const replacements=[
    ['เพื อ','เพื่อ'],['เพื','เพื่อ'],['เปลี ยน','เปลี่ยน'],['เครื อง','เครื่อง'],['เรื อง','เรื่อง'],['เชื อม','เชื่อม'],
  ...
  out=replaceTrack(out,/([ก-ฮ])\s+([ก-ฮ])(?=(?:ว่า|ที่|แล้ว|อยู่|ด้วย|จาก|ให้|ของ|การ|งาน|ระบบ|เครื่อง|เอกสาร|ขั้นตอน|บ่อย|ข้อมูล|ข้อความ|ลิงก์|ประเมิน|อุปกรณ์|ทดแทน|เพื่อ))/g,'$1$2');
  return out;
}
...
function calculateConfidence(raw,cleaned){
  ...
  if(!rawLen)return 0;
  const weird=((raw||'').match(/[�ƟθϴƩΣÉÊÈË○●$]/g)||[]).length;
